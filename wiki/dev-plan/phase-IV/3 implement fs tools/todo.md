# Задача: «implement fs tools»
Необходимо закончить реализацию FSTools в `Tools\harness\agents\tools\fs_tools.py`.

Запускать код не нужно. Только исправить до рабочего варианта. Проверять буду самостоятельно.
...
4. О результатах отпишись в `result.md` в формате `было→стало(почему)` (создай файл в той же папке где и `todo.md` для задачи).
