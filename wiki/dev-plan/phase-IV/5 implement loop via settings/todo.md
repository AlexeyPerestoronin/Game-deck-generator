...
Необходимо проанализировать актуальный код обвязки для AI агентов Grok и Gemini (`Tools\harness\agents\grok.py`, `Tools\harness\agents\gemini.py`).
Рабочий код изменять нельзя, но можно и нужно создать в папке с задачей файл `implement.diff`, использовав который можно будет применить предложенные изменения.
