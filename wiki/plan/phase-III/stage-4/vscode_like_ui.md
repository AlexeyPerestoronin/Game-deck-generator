...
1. Необходимо исправить баг c pdf preview:
   - задача на доработку №2 исправила проблему частично:    
     - в Chrome страницы в pdf preview белые (так и должно быть)
     - в Edge страницы в pdf preview белые (так и должно быть)
     - в FireFiz страницы в pdf preview чёрные → надо исправить → страницы должны быть тоже белыми!
   + условия к решению такие же как и для доработки №2.
2. Секции `Local` и `Global` отображаются правильно (ошибка исправлена), но не правильно сворачиваются → надо исправить:
   - Как должно быть:
     - Как в VSCode
       - обе секции всегда должны занимать всё пространство на боковой панели, если хотя бы одна секция раскрыта
       - если все секции свёрнуты, они должны располагаться сверху боковой панели одна под другой (не одна над другой, как сейчас, что делает невозможным повторное раскрытие после сворачивания)

# Задача на доработку №4:
1. Все проблемы решились, кроме с проблемы с чёрными страницам в pdf preview в FireFox → надо исправить.
   - разрешаю, если необходимо, вносить изменения в prepare_pdf_web

# Задача на доработку №5:
1. Проблема с чёрными страницам в pdf preview в FireFox не решена = страницы по прежнему черные → надо исправить:
   - я откатил все изменения связанные с доработкой №4; 
   - а также нашёл commit где решалась и решилась подобная же проблема (см. 22caa3fa405a25c14ec3d5d46e9b8f77d41b7981), возможно это как-то поможет;
   - возможно также поможет посмотреть изменения в предыдущих 2 комитах от указанного (в рамках этих комитов добавлялось splite preview)
