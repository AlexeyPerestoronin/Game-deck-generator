# Задача
Необходимо поддержать возможность изменять стиль иконки для кнопки путём прямого редактирования файла.

Детали реализации:
Все иконки должны храниться в папке `deck_gen_wasm/icons/buttons`.
Для каждой кнопки должна быть выделена отдельная подпапка два файла: активна, неактивна.

Пример для кнопки загрузки директории с игрой из локальной файловой системы:
```
deck_gen_wasm/
    buttons/
        load_game/ # возможно изменения названия на более подходящее
            off.drawio.png    # стиль кнопки в отсутствии наведения курсора
            on.drawio.png     # стиль кнопки при наведения на неё курсора мышки
            click.drawio.png  # стиль кнопки при нажатии

```
Необходимо создать иконки для всех имеющихся кнопок во всех вариантах (off, on, click) в минимальном но достаточном размере по высоте и ширине для красивого отображения на графическом интерфейсе.
Размеры всех иконок должен быть одинаковыми.
