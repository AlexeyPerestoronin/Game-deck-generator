# Задача
Необходимо подержать адекватное масштабирование интерфейса рабочего пространства по двум следующим пунктам:
1. Область рабочего пространства в котрой располагаются управляющие кнопки должна всегда полностью влезать в доступную высоту рабочей вкладки, чтобы устранить необходимость перематывания вниз с целью нажать на кнопку `Clear`.
2. Область рабочего пространства в которой отображается дерево файлов рабочего пространства должна адаптировать под доступную высоту рабочей вкладки даже если при раскрытии директории с большим количеством файлов не все эти файлы помещаются в экран (в этом случае пользователь должен обладать возможностью скролинга в дереве директорий, как если это реализовано в VSCode во вкладке Explorer)
