# Задача: «open tabs change range»

Необходимо добавить возможность изменять порядок открытых вкладок в рабочей области (подобно тому, это позволяет делать VSCode путём перетаскивая открытых вкладок через операция "нажал и перетащил на нужное место").
