# Задача: «open tabs rearrange» — ОБЪЕДИНЕНА

**Эта задача объединена с close all tabs из-за сильного пересечения по файлам (tab.rs, pane.rs, workspace split/state, menus).**
...
3. Какой код использовать для анализа:
   ...
   - прочие файлы и папки репозитория ИГНОРИРУЙ (в т.ч. app.rs, bars/, buttons/, explorer/*, conf/*, menus/*, fs/*, tooltips/*);
