...
## Доработка №4 (жёсткий подход — принудительный reload интерфейса по клику)
было: предыдущие попытки исправить live-реактивность (snapshot-откат в app.rs, move || в меню/табах/кнопках) не решили проблему по верификации пользователя — тексты chrome обновлялись только после ручного F5.
стало: в `on:click` LocaleButton (после `change_locale()` + `current.set` + persist) добавлен принудительный `if let Some(win) = web_sys::window() { let _ = win.location().reload(); }`.
почему: прямой запрос пользователя на "жёсткий подход: пусть весь интерфейс принудительно перезагружается при нажатии на кнопку локализации". change_locale уже делает persist в localStorage по LOCALE_KEY; reload заставляет приложение стартовать заново, где load_persisted() + ensure + apply + все view! прочитают новый Locale при первой инициализации. Полностью обходит любые проблемы с RwSignal подписками в Leptos. Изменение минимально — 1 use + ~5 строк только в buttons/locale.rs (в Scope).

- После: cargo test -p deck_gen_wasm_ui -p deck_gen_wasm_locale, cargo check, trunk build — зелёные.
- Поведение: клик по Locale → смена + persist + автоматический reload вкладки → весь интерфейс (Games, New File/Folder, empty, меню, табы, модалки, статусы новых действий, title, lang) приходит на выбранном языке без необходимости вручную жать F5.
- Побочный эффект: это полный reload (как F5), теряется только не-персистентное состояние; workspace восстанавливается из localStorage+IDB как и раньше.
- Соответствует "минимальные изменения", "только Scope", "простой и прямолинейный код".
