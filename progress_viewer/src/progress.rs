...
/// Interface for progress reporting. Implementors receive `set` calls with
/// values in 0..=100 (clamped by caller if needed).
pub trait ProgressHandler {
    /// Record progress percentage (0.0..=100.0).
    fn set(&self, pct: f32);
}
...
impl ProgressHandler for Progress {
    fn set(&self, pct: f32) {
        Progress::set(self, pct);
    }
}

/// A no-op implementation of [`ProgressHandler`] for places that must satisfy
/// the signature but do not want to report (e.g. engine trait adapters).
#[derive(Clone, Copy, Debug, Default)]
pub struct NoopProgress;

impl ProgressHandler for NoopProgress {
    fn set(&self, _pct: f32) {}
}
