//! Percent reporting for long actions (CLI + WASM).
//!
//! [`Progress`] stores a callback plus an optional paint hook (WASM only).
//! The three macros mark statement ranges and `paint().await` after each `set`
//! so a WASM UI can redraw.
//!
//! Public API exposes [`ProgressHandler`] trait for extension points in
//! `deck_gen`, `prepare_pdf_host`, and `prepare_pdf_web`.
...
pub use progress::{NoopProgress, Progress, ProgressHandler};
...
mod tests {
    ...
    #[test]
    fn noop_progress_handler() {
        let noop = NoopProgress;
        noop.set(42.0);
        noop.set(-1.0);
        // no panic, just ignores
    }
}
