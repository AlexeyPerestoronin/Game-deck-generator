//! Deck model after JSON5 substitution.
//!
//! [`Deck`] is the in-memory view of one `data.json5`: normalised deck fields,
//! extra top-level keys, and the raw `cards` array. Card objects for templates
//! are built on demand via [`Deck::cards`] so slug / life-effects stay next to
//! [`crate::card`]. Paths (`directory`, `data_path`) are whatever the
//! [`FileSystem`] canonicalized.

use std::path::{Path, PathBuf};

use serde_json::{Map, Value};

use crate::card::{card_from_row, normalize_map};
use crate::conf::{output_dir_for_name, Conf};
use crate::error::{Error, Result};
use crate::fs::FileSystem;
use crate::load::DataManager;

/// One loaded deck: fields for Jinja plus the source card rows.
pub struct Deck {
    /// Deck name as declared in data.json5 `name` (unique within its game; independent of folder).
    pub name: String,
    /// Game id owning this deck (from its game.json5).
    pub game_id: String,
    /// Directory of `data.json5` (first Jinja search path).
    pub directory: PathBuf,
    /// Canonical data file path (error reporting).
    pub data_path: PathBuf,
    /// Normalised `deck` object merged with extra top-level keys.
    pub fields: Map<String, Value>,
    card_rows: Vec<Value>,
}

impl Deck {
    /// Split a substituted payload into deck fields, card rows, and extras.
    pub fn from_manager<F>(manager: &DataManager<'_, F>) -> Result<Self>
    where
        F: FileSystem + ?Sized,
    {
        let payload = manager.get_data()?;
        let (deck_fields, card_rows, extras) = split_payload(&payload, &manager.path)?;

        let mut data = normalize_map(&deck_fields);
        let extra_fields = normalize_map(&extras);
        for (key, value) in &extra_fields {
            data.insert(key.clone(), value.clone());
        }

        let name = extra_fields
            .get("name")
            .and_then(Value::as_str)
            .filter(|name| !name.is_empty())
            .ok_or_else(|| Error::file(&manager.path, "must contain 'name'"))?
            .to_string();
        if !data.contains_key("view") {
            return Err(Error::file(&manager.path, "must contain 'view'"));
        }
        data.insert("key".into(), Value::String(name.clone()));

        Ok(Self {
            name,
            game_id: String::new(),
            directory: manager.directory.clone(),
            data_path: manager.path.clone(),
            fields: data,
            card_rows,
        })
    }

    /// Template filename for `view.face` or `view.back`.
    pub fn template_for(&self, side: &str) -> Result<String> {
        self.fields
            .get("view")
            .and_then(Value::as_object)
            .and_then(|view| view.get(side))
            .and_then(Value::as_str)
            .filter(|name| !name.is_empty())
            .map(str::to_string)
            .ok_or_else(|| {
                Error::file(&self.data_path, format!("view.{side} is required"))
            })
    }

    /// `_autogenerated/…` directory for this deck under `loaded`.
    pub fn output_dir(&self, loaded: &Conf) -> Result<PathBuf> {
        output_dir_for_name(loaded, &self.game_id, &self.name)
    }

    /// Card width from deck fields (string or number; `0.0` if missing).
    pub fn card_width_mm(&self) -> f64 {
        number(&self.fields["card_width_mm"])
    }

    /// Card height from deck fields (string or number; `0.0` if missing).
    pub fn card_height_mm(&self) -> f64 {
        number(&self.fields["card_height_mm"])
    }

    /// `game_name` field, or `""` if absent.
    pub fn game_name(&self) -> String {
        self.fields
            .get("game_name")
            .and_then(Value::as_str)
            .unwrap_or("")
            .to_string()
    }

    /// Materialize card objects: `card_*` defaults + row + `index` / `slug`.
    pub fn cards(&self) -> Vec<Value> {
        let defaults: Map<String, Value> = self
            .fields
            .iter()
            .filter(|(key, _)| key.starts_with("card_"))
            .map(|(key, value)| (key.clone(), value.clone()))
            .collect();
        self.card_rows
            .iter()
            .enumerate()
            .map(|(index, row)| card_from_row(index + 1, row, &defaults))
            .collect()
    }
}

fn split_payload(
    payload: &Map<String, Value>,
    path: &Path,
) -> Result<(Map<String, Value>, Vec<Value>, Map<String, Value>)> {
    let deck_fields = payload
        .get("deck")
        .and_then(Value::as_object)
        .cloned()
        .ok_or_else(|| Error::file(path, "must contain object 'deck'"))?;
    let card_rows = payload
        .get("cards")
        .and_then(Value::as_array)
        .cloned()
        .ok_or_else(|| Error::file(path, "must contain list 'cards'"))?;
    let extras: Map<String, Value> = payload
        .iter()
        .filter(|(key, _)| *key != "deck" && *key != "cards")
        .map(|(key, value)| (key.clone(), value.clone()))
        .collect();
    Ok((deck_fields, card_rows, extras))
}

fn number(value: &Value) -> f64 {
    match value {
        Value::Number(n) => n.as_f64().unwrap_or(0.0),
        Value::String(text) => text.parse().unwrap_or(0.0),
        _ => 0.0,
    }
}
