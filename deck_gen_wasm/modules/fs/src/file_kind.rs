...
/// Files that must go to IndexedDB (large binaries) instead of the localStorage JSON snapshot.
pub fn is_persisted_as_binary(path: &str) -> bool {
    matches!(kind_of(path), FileKind::Image | FileKind::Pdf)
}
